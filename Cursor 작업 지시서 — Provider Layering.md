...
> 이 문서는 **React + 단일 `App.js` 모놀리식 구조**에서 작업하는 Cursor AI를 위한
> 최상위 업무 원칙이다.  
...
## 2. Provider 레벨 분류 (개념적 레이어)

Cursor는 App.js 안에 존재하는 모든 Provider를 아래 다섯 레벨로 분류한다.  
구체 라이브러리 이름 대신, **역할로만 생각한다.**

1. **라우팅 레벨**  
   - URL, 페이지 전환, 라우트 매칭 등을 담당하는 Provider.

2. **스타일/테마 레벨**  
   - 전역 스타일, 테마, 디자인 시스템을 주입하는 Provider.

3. **서버 상태/데이터 레벨**  
   - 서버와의 통신, 캐싱, 비동기 데이터 로딩/동기화를 담당하는 Provider.

4. **인증/사용자 정보 레벨**  
   - 로그인 여부, 사용자 정보, 권한 등을 관리하는 Provider.

5. **전역 UI 상태/알림 레벨**  
   - 알림, 모달, 전역 토스트, 레이아웃 상태 등을 관리하는 Provider.

> 각 Provider가 이 다섯 레벨 중 어디에 속하는지 **먼저 분류한 뒤**,  
> “바깥에서 안쪽으로” 레벨 순서를 정한다.

---

## 3. 이상적인 레이어링 순서 (개념)

이상적인 감싸는 순서는 개념적으로 다음과 같다.
...
Cursor는 **현재 App.js의 Provider 구조가 이 순서를 어느 정도 따라가도록** 조정한다.  
단, 프로젝트 특성상 다른 순서가 더 타당하면, 그 이유를 먼저 설명한 후 예외를 둘 수 있다.
